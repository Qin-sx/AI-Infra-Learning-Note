...
1. 接收 `bootstrap_room, status, prefill_rank` 消息。
2. 更新本地请求状态（`request_status` / `prefill_response_tracker`）。
3. 通知上层调度器（由后续 `poll()`/`pop_transferred()` 消费状态）。

下面是对应的代码细节展开。

prefill 侧在传输线程里，在关键节点调用 `sync_status_to_decode_endpoint(...)` 上报状态：
...
在 `transfer_worker(...)` 中，至少有三类路径会触发上报：

1. 发现远端 session 异常，直接上报 `KVPoll.Failed`。
2. `send_kvcache(...)` / `send_kvcache_slice(...)` 返回失败，上报 `KVPoll.Failed`。
3. 最后一块数据发送结束后，按 `all(polls)` 汇总为 `KVPoll.Success` 或 `KVPoll.Failed` 并上报。

decode 侧在 `start_decode_thread()` 的 `decode_thread` 中持续 `recv_multipart()` 收消息并更新状态：
...
if status == KVPoll.Success:
    ...
    if len(self.prefill_response_tracker[bootstrap_room]) == expected_response_num:
        self.update_status(bootstrap_room, KVPoll.Success)
elif status == KVPoll.Failed:
    self.record_failure(bootstrap_room, ...)
    self.update_status(bootstrap_room, KVPoll.Failed)
```

这意味着 decode 侧不是直接调用 mooncake 的传输 API 去“拉取”状态，而是消费 prefill 主动上报的状态消息，再写入 `request_status`。
...
这里的关键点是：`poll()` 自己不调用 `engine.batch_transfer_sync()` 或 `engine.get_batch_transfer_status()`。它只读取 `kv_mgr.check_status(...)`，而这个状态正是上一小节 decode 线程处理 ZMQ 上报后写入的结果。
